...
`,
	...
	for _, res := range results {
		...
		case "Multifile_variants_a_esnext.txtar":
			...
			if !strings.Contains(res.Content, "exec tscc --report-all-diagnostics --target esnext --lib esnext,dom --out-js a.js a.ts") {
				t.Errorf("Expected a_esnext to execute a.ts with command 'exec tscc --report-all-diagnostics --target esnext --lib esnext,dom --out-js a.js a.ts', got:\n%s", res.Content)
			}
		...
		case "Multifile_variants_b_esnext.txtar":
			if !strings.Contains(res.Content, "exec tscc --report-all-diagnostics --target esnext --lib esnext,dom --out-js b.js b.ts") {
				t.Errorf("Expected b_esnext to execute b.ts with command 'exec tscc --report-all-diagnostics --target esnext --lib esnext,dom --out-js b.js b.ts', got:\n%s", res.Content)
			}
		}
	}
}
