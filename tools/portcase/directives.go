...
func TranslateDirectives(directives map[string]string, outputBaseName string) ([]string, error) {
	...
	for _, key := range keys {
		...
		case "nouncheckedsideeffectimports":
			if strings.ToLower(value) == "true" || value == "" {
				flags = append(flags, "--no-unchecked-side-effect-imports")
			} else if strings.ToLower(value) == "false" {
		...
		case "noimplicitreturns":
			if strings.ToLower(value) == "true" || value == "" {
				...
				flags = append(flags, "--no-no-implicit-returns")
			}
		case "nopropertyaccessfromindexsignature":
			if strings.ToLower(value) == "true" || value == "" {
				flags = append(flags, "--no-property-access-from-index-signature")
			} else if strings.ToLower(value) == "false" {
		...
		case "usedefineforclassfields":
			if strings.ToLower(value) == "true" || value == "" {
				...
				flags = append(flags, "--no-use-define-for-class-fields")
			}
