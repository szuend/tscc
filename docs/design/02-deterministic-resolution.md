...
4.  **Implement `FSJail`**: A `vfs.FS` middleware that blocks configuration files, `node_modules`-style directories (including `bower_components`, `jspm_packages`), makes `Realpath` the identity function, and returns a caller-pinned `UseCaseSensitiveFileNames()`. Covers `Stat`, `FileExists`, `ReadFile`, `GetAccessibleEntries`, `WalkDir`.
